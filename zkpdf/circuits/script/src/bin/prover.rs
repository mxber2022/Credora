...
async fn main() {
    ...
    let prover = std::env::var("SP1_PROVER").unwrap_or_else(|_| "cpu".to_string());
    
    // Only require network key if using network prover
    if prover == "network" {
        let key = std::env::var("NETWORK_PRIVATE_KEY").unwrap_or_default();
        assert!(
            key.starts_with("0x") && key.len() > 10,
            "Invalid or missing NETWORK_PRIVATE_KEY for network prover"
        );
    }
