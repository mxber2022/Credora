...
function work() {
    // Payslip.pdf is the file that is going to be signed
    var sourcePath = path.join(__dirname, '/../../../resources/Payslip.pdf');
    ...
    // Load the document into PDF-LIB (ignore encryption if present)
    PDFDocument.load(pdfBuffer, { ignoreEncryption: true }).then(function (pdfDoc) {
        // Add a placeholder for a signature.
        pdflibAddPlaceholder({
            pdfDoc: pdfDoc,
            reason: 'Digital signature for payslip document',
        ...
        pdfDoc.save().then(function (pdfWithPlaceholderBytes) {
            ...
            signpdf
                ...
                .then(function (signedPdf) {
                    ...
                    var targetPath = path.join(__dirname, '/../output/payslip-signed.pdf');
                    fs.writeFileSync(targetPath, signedPdf);
                    console.log('Payslip.pdf has been successfully signed and saved as payslip-signed.pdf');
                })
